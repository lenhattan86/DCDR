...
for idx = 1:MC_samples      
    [optVal, rt_energy_cost, queuing_delay_cost, network_delay_cost, m_left, lambda_left, isSuccessful] = ...                    
        geo_load_balance(q_l_minus, w_rSamples(:,idx), p_rSamples(:,idx), L_rSamples(:,idx), mu ,M , beta ,pi_ij);
    if isSuccessful<0 
        idx
        q_l_minus
    ...
    [optVal, rt_energy_cost, queuing_delay_cost, network_delay_cost, m_right, lambda_right, isSuccessful] = ...
        geo_load_balance(q_l_plus, w_rSamples(:,idx), p_rSamples(:,idx), L_rSamples(:,idx), mu ,M , beta ,pi_ij);
    if isSuccessful<0
        idx
        q_l_plus
