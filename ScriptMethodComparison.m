...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%GENERATE_DATA = 0;
GEN_FILE = 'scriptMethodComparison.mat';
extraName = 'Fig2';
...
NoLT = [0 rt_energy_cost_NoLT queuing_delay_cost_NoLT network_delay_cost_NoLT]; 
fixedLT = [exp_cost_fixedLT, rt_energy_cost_fixedLT, queuing_delay_cost_fixedLT, network_delay_cost_fixedLT];                

pred_alg = [q_l_pred'*p_l rt_energy_cost_pred queuing_delay_cost_pred network_delay_cost_pred];

expect_SGEA = [q_l_pred'*p_l rt_energy_cost_SGEA queuing_delay_cost_SGEA, network_delay_cost_SGEA];   

if ~SKIP_PLOTTING     
    figure;
%     y = [NoLT_NoGLB; fixedLT_NoGLB; NoLT; fixedLT; pred_alg; expect_SGEA; exp_offline_cost];
    y = [NoLT_NoGLB(1:2); fixedLT_NoGLB(1:2); NoLT(1:2); fixedLT(1:2); pred_alg(1:2); expect_SGEA(1:2); exp_offline_cost(1:2)];
...
%% V. Saving Results
performance_gap = (sum(pred_alg(1:2)) - sum(exp_offline_cost(1:2)))/sum(exp_offline_cost(1:2))*100;
if ~exist('extraName')
    extraName='';
end
extraName = [extraName '_gap' num2str(round(performance_gap))];
